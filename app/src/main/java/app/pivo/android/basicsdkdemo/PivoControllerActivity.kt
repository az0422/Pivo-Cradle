...
class PivoControllerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //show pivo version
        version_view.text =
            "Pivo Type: ${PivoSdk.getInstance().version.pivoType}\nVersion: ${PivoSdk.getInstance().version.version}"

        //rotate continuously to left
        btn_left_con_turn.setOnClickListener {
            PivoSdk.getInstance().turnLeftContinuously(speedList[position])
        }

        //rotate to left
        btn_left_turn.setOnClickListener {
            PivoSdk.getInstance().turnLeft(getAngle(), speedList[position])
        }

        //rotate continuously to right
        btn_right_con_turn.setOnClickListener {
            PivoSdk.getInstance().turnRightContinuously(speedList[position])
        }

        //rotate to right
        btn_right_turn.setOnClickListener {
            PivoSdk.getInstance().turnRight(getAngle(), speedList[position])
        }
        ...
        //speed list view
        speed_list_view.adapter =
            ArrayAdapter<Int>(this, android.R.layout.simple_spinner_item, speedList)
