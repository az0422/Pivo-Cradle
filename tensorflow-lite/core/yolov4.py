...
from core.yolo_custom import YOLOv4_custom

from core.cfg_parsor import model_parse
import re
...
#YOLO_TYPE = 0 #0: original, 1: custom

def layer_convolutional(input_layer, config, layers):
    return common.convolutional(input_layer,
                                (int(config["size"]), int(config["filters"])),
                                downsample = True if config["stride"] == "2" else False,
                                bn = True if config["batch_normalize"] == "1" else False,
                                activate = False if config["activation"] == "linear" else True
                                )
    
def layer_route(input_layer, config, layers):
    layers_index = eval("[%s]" % config["layers"])
    
    if len(layers_index) > 1:
        layers_select = []
        
        for index in layers_index:
            layers_select.append(layers[index])
        
        input_layer = tf.concat(layers_select, axis=-1)
    
    else:
        input_layer = layers[layers_index[0]]
    
    if config["groups"] != '0':
        input_layer = common.route_group(input_layer, int(config["groups"]), int(config["group_id"]))
    
    return input_layer

def layer_maxpool(input_layer, config, layers):
    return tf.keras.layers.MaxPool2D(int(config["size"]), int(config["stride"]), 'same')(input_layer)
